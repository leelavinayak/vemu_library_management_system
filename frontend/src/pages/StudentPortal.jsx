...
import { BookOpen, Bell, Clock, AlertTriangle, CheckCircle, Edit2, Save, X, Search, Trash2 } from 'lucide-react';
...
const StudentPortal = () => {
  const links = [
    ...
    { path: '/student/fines', label: 'Fines' },
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <BookOpen size={48} />
          <p>No books found{searchTerm ? ` for "${searchTerm}"` : ''}.</p>
        </div>
      ) : (
        <div className="grid">
          {filtered.map(book => (
...
const StudentFines = () => {
  ...
  return (
    <div>
      <div className="section-header"><h2>Fine Details</h2></div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '1.25rem', marginBottom: '2rem' }}>
        <div className="stat-card">
          <div className="stat-label">Total Pending Fine</div>
          <div className="stat-num" style={{ color: totalFine > 0 ? 'var(--danger)' : 'var(--success)' }}>₹{totalFine}</div>
          <AlertTriangle size={40} className="stat-icon" style={{ color: 'var(--danger)' }} />
        </div>
        <div className="stat-card">
          <div className="stat-label">Overdue Books</div>
          <div className="stat-num">{history.filter(t => t.status === 'active').length}</div>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ position: 'relative', maxWidth: '400px' }}>
          <Search size={18} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-light)' }} />
          <input type="text" placeholder="Search fines by book title..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{ paddingLeft: '42px' }} />
        </div>
      </div>

      {history.length === 0 ? (
        <div className="empty-state"><CheckCircle size={48} style={{ color: 'var(--success)', opacity: 1 }} /><p>You have no pending fines. Great job!</p></div>
      ) : filtered.length === 0 ? (
        <div className="empty-state"><Search size={48} /><p>No matching fines found.</p></div>
      ) : (
        <div className="table-container">
          <table>
            <thead><tr><th>Book</th><th>Fine Amount</th><th>Status</th></tr></thead>
            <tbody>
              {filtered.map(t => (
                <tr key={t._id}>
                  <td><strong>{t.book?.title}</strong></td>
...
export const Notifications = () => {
  ...
  const handleClearAll = async () => {
    setModalConfig({ isOpen: true, type: 'all', id: null });
  };
  ...
  const executeDelete = async () => {
    try {
      if (modalConfig.type === 'all') {
        ...
        await api.delete(`/api/notifications/${modalConfig.id}`);
    ...
    <div>
      <div className="section-header">
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          {unread.length > 0 && <span style={{ background: '#fee2e2', color: '#991b1b', padding: '4px 12px', borderRadius: '99px', fontSize: '0.8rem', fontWeight: 600 }}>{unread.length} Unread</span>}
          {notifications.length > 0 && (
            <button onClick={handleClearAll} className="ghost" style={{ color: 'var(--danger)', fontSize: '0.8rem', padding: '6px 12px' }}>
      ...
      {notifications.length === 0 ? (
        ...
        <>
          {unread.length > 0 && (
            <>
              <p style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>Unread</p>
              {unread.map(n => (
          ...
          {read.length > 0 && (
            <>
              <p style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '1.25rem 0 0.75rem' }}>Read</p>
              {read.map(n => (
      ...
      <ConfirmModal
        ...
        title={modalConfig.type === 'all' ? 'Clear All Notifications' : 'Delete Notification'}
        ...
        confirmText={modalConfig.type === 'all' ? 'Yes, Clear All' : 'Delete'}
      />
    </div>
  );
};
